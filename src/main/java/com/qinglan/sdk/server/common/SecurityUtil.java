...
public abstract class SecurityUtil {
    ...
    public String encryptToBase64(String key, String src, String encoding) {
        try {
            ...
            return encoder.encodeToString(encrypt(key.getBytes(), src.getBytes(encoding)));
...
//					.getBytes(encoding)));
    ...
    public String decryptFromBase64(String key, String src, String encoding) {
        try {
            ...
            return new String(decrypt(key.getBytes(), decoder.decode(src)), encoding);
...
//			ex.printStackTrace();
    ...
    public String encryptToBase64DefaultEncoding(String key, String src) {
        try {
            Base64 encoder = new Base64();
            return encoder.encodeToString(encrypt(key.getBytes(), src.getBytes()));
//            BASE64Encoder encoder = new BASE64Encoder();
//            return encoder.encode(encrypt(key.getBytes(), src.getBytes()));
    ...
    public String decryptFromBase64DefaultEncoding(String key, String src) {
        try {
            Base64 decoder = new Base64();
            return new String(decrypt(key.getBytes(), decoder.decode(src)));
//            BASE64Decoder decoder = new BASE64Decoder();
//            return new String(
//                    decrypt(key.getBytes(), decoder.decodeBuffer(src)));
        } catch (Exception ex) {
...
//                byte[] b = new sun.misc.BASE64Decoder().decodeBuffer(str);
            Base64 decoder = new Base64();
            byte[] b = decoder.decodeBase64(str);
            return new String(b);
        }
    }
...
//            return new sun.misc.BASE64Encoder().encode("N".getBytes());
            return encoder.encodeToString("N".getBytes());
        }
    }
