...
import com.qinglan.sdk.server.platform.utils.ibei.SignHelper;
import com.qinglan.sdk.server.platform.utils.lewan.util.MD5Util;
import com.qinglan.sdk.server.platform.utils.qq.JSONException;
import com.qinglan.sdk.server.platform.utils.qq.JSONObject;
import com.qinglan.sdk.server.platform.utils.six7.Six7payUtils;
import com.qinglan.sdk.server.service.OrderService;
import com.qinglan.sdk.server.utils.RedisUtil;
import com.qinglan.sdk.server.platform.service.PlatformServicePartTwo;
...
public class PlatformServicePartTwoImpl implements PlatformServicePartTwo {
    ...
    public String verifyQinmuSession(QinmuSession session) {
        ...
        try {
            ...
            sb.append(session.getAuthorizeCode());
            sb.append("&").append("app_key=").append(session.getAppKey());
            sb.append("&").append("jh_sign=").append(app_secret);
            sb.append("&").append("time=").append(time);
            String sigin = MD5.encode(sb.toString());
            Map<String, Object> params = new HashMap<String, Object>();
            params.put("app_key", session.getAppKey());
            params.put("authorize_code", session.getAuthorizeCode());
    ...
    public String verifyCangluanSession(CangluanSession session) {
        ...
        try {
            ...
            PlatformGame platformGame = basicRepository.getByPlatformAndAppId(platformId, ygAppId);
    ...
    public String verifySix7Session(Six7Session session) {
        Map<String, String> result = new HashMap<String, String>();
        try {
            PlatformGame platformGame = basicRepository.getByPlatformAndAppId(Integer.valueOf(session.getPlatform()), Long.valueOf(session.getYgAppId()));
            if (platformGame == null) {
                result.put("code", "1");
                result.put("msg", "platorm not unition");
                return JsonMapper.toJson(result);
    ...
    public String verifyXmwSession(XmwSession xmwSession) {
        ...
        try {
            PlatformGame platformGame = basicRepository.getByPlatformAndAppId(Integer.valueOf(xmwSession.getPlatformId()), Long.valueOf(xmwSession.getYgAppId()));
            if (platformGame == null) {
                result.put("code", "1");
                result.put("msg", "platorm not unition");
                return JsonMapper.toJson(result);
    ...
    public String createXmwOrder(XmwOrderSession xmwOrderSession) {
        ...
        try {
            PlatformGame platformGame = basicRepository.getByPlatformAndAppId(Integer.valueOf(xmwOrderSession.getPlatformId()), Long.valueOf(xmwOrderSession.getYgAppId()));
            if (platformGame == null) {
                result.put("code", "1");
                result.put("msg", "platorm not unition");
                return JsonMapper.toJson(result);
    ...
    public String verifyDlSession(DlSession dlSession){
        ...
        PlatformGame platformGame = basicRepository.
                getByPlatformAndAppId(Integer.valueOf(dlSession.getPlatformId()), Long.valueOf(dlSession.getYgAppId()));
        if (platformGame == null) {
...
//            String productName = URLDecoder.decode(dto.getProductName(), "UTF-8");
            StringBuilder sb = new StringBuilder();
    ...
    public String verifyGuangFan(HttpServletRequest request){
        try {
            ...
            Map map = JsonMapper.toObject(sbr.toString(), Map.class);
            ...
            String dataJson = JsonMapper.toJson(dataObj);
            Map dataMap = JsonMapper.toObject(dataJson, Map.class);
